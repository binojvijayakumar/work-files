...
function taxathand_article_generator(ctrlid, resultCtrl, itemsPerRow, actionedItemsCtrl, filterIDCtrl, filterStartDate, filterEndDate) {
    if (taxathandArticlesResponse && taxathandArticlesResponse.length &&
        ...
        var k = 0;
        var actionedItems = [];
        if (actionedItemsCtrl && actionedItemsCtrl.split('_').length == 3) {
            actionedItems = $('#' + actionedItemsCtrl).val().split(',');
        }

...
function taxathand_article_generator(ctrlid, resultCtrl) {
    if (taxathandArticlesResponse && taxathandArticlesResponse.length && ctrlid && resultCtrl && ctrlid.split('_').length == 3 && resultCtrl.split('_').length == 3) {
        ...
        var k = 0;
        for (var i = 0; i < Math.ceil(taxathandArticlesResponse.length / 2); i++) {
            node += '<tr>';
            for (var j = 0; j < 2; j++) {
                var v = taxathandArticlesResponse[k++];
                if (v) {
                    node += '<td><div class="card"> <div class="upper" style="background-image:url(' + v.imagePath + ')"> <div class="upper-banner text-container"><a target="_blank" href="' + v.sharingWebURL + '"> <p class="upper-text">' + v.title + '</p></a> </div> </div> <div class="middle"> <div class="text-container"> <p class="middle-text"> ' + v.teaserText + ' </p> </div> </div> <div class="lower"> <div class="text-container"> <label for="followUp">Follow up needed ? Yes/No</label> <input name="followUp" id="followUp" type="checkbox" uuid="' + v.uuid + '" /> </div> </div> </div></td>';
                }
            }
            node += '</tr>';
        }
